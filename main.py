...
def diff_to_sec(time1, time2):
    ...
    time = transform_time(time1) - transform_time(time2)
    ...
    for user_id, activities in activities_by_user.items():  # O(n) x O(m) = O(n x m)
        ...
        for activity in activities:
            if (
                ...
                user_sessions["user_sessions"][user_id][-1][
                    "duration_seconds"
                ] = diff_to_sec(
                    user_sessions["user_sessions"][user_id][-1]["ended_at"],
...
if __name__ == "__main__":
    ...
    user_activities = activities_by_user(initial_data)
